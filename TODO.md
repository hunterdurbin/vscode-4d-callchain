...
| 13 | Tree-sitter / proper parser — *in progress* | XL | New `@4d/parser-4d` workspace ships a hand-written tree-sitter grammar (10 corpus-test files, 131 tests passing) compiled to WASM. `@4d/core` consumes it via `web-tree-sitter`. Behind `FOURD_PARSER=treesitter` env flag — sync parser dispatch in `fileParser.ts:130` falls through to the legacy regex parser when the flag is off or init hasn't run. Mini-4d parity: 76/76 symbols (100 %), 106/104 calls (slight over-emit, +2 from builtins like `Type` the regex parser filters elsewhere). All 255 existing tests pass under both modes. WASM packaging needs no per-platform prebuilds (the original plan budgeted darwin-x64/arm64, linux-x64/arm64, win32-x64 native binaries — pivoted away because Node 25's V8 headers broke the native build path). Remaining work to flip the default: plumb the project's builtin / constants / plugin-command sets into `cstVisitor` so it can filter / promote those refs; rewrite `blockScanner.ts`/`variableScanner.ts` as CST walks; bump `INDEX_VERSION`; delete `callExtractor.ts`. Then Phase 8 (incremental reparse via `textDocument/didChange` deltas) becomes worth pursuing. |
