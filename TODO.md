...
| 13 | Tree-sitter / proper parser — *in progress* | XL | New `@4d/parser-4d` workspace ships a hand-written tree-sitter grammar (10 corpus-test files, 131 tests passing) compiled to WASM. `@4d/core` consumes it via `web-tree-sitter`. Behind `FOURD_PARSER=treesitter` env flag — sync parser dispatch in `fileParser.ts:130` falls through to the legacy regex parser when the flag is off or init hasn't run. Mini-4d parity: 97 % symbols (74/76), 92 % calls (96/104). All 255 existing tests pass under both modes. WASM packaging needs no per-platform prebuilds (the original plan budgeted darwin-x64/arm64, linux-x64/arm64, win32-x64 native binaries — pivoted away because Node 25's V8 headers broke the native build path). Remaining work: bring call parity to ≥ 99.5 % (FormRef detection from `DIALOG`/`Open form window` arg inspection, plugin-command resolution via the project's plugin-manifest set, ConstantRef via the project's constants set), flip the default, delete `callExtractor.ts`, rewrite `blockScanner.ts`/`variableScanner.ts` as CST walks, then bump `INDEX_VERSION`. |
